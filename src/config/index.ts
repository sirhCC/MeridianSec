...
export function loadConfig(configPath = 'canary.config.json'): AppConfig {
  ...
  const merged = {
    database: { provider: 'sqlite', url: process.env.DATABASE_URL || 'file:./data/canary.db', ...(fileRaw.database||{}) },
    alerting: { slackWebhook: process.env.SLACK_WEBHOOK_URL, webhookSignatureSecret: process.env.ALERT_SIGNING_KEY, ...(fileRaw.alerting||{}) },
    cloudtrail: { mode: 'mock', pollIntervalMs: 5000, ...(fileRaw.cloudtrail||{}) },
